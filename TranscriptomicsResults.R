...
# load TCGA PANCAN12 RNA-seq data (downloaded from https://xenabrowser.net/datapages/?dataset=TCGA.PANCAN12.sampleMap%2FPanCan12.3602-corrected-v3_syn1715755&host=https%3A%2F%2Flegacy.xenahubs.net&addHub=https%3A%2F%2Flegacy.xenahubs.net&removeHub=https%3A%2F%2Fxena.treehouse.gi.ucsc.edu%3A443)
# data were already preprocessed and imputed 
...
#obtained from the altayg/ganet package -> corum = fl2a(ganet::corum)
load("data/corum")

# load Reactome pathway annotations obtained from the graphite package (access date 06/14/2019)
# the follwing filters were applied to subselect the pathways:
# 1. pathway contains at least 50 genes and at most 1,000 genes 
...
# set to F if you want to regenerate all results from scratch
...
# no_cores = 3
# cl = makeCluster(no_cores,outfile= "cutopt_logfile")
# clusterExport(cl, setdiff(ls(),c("cl","no_cores")))
# clusterEvalQ(cl, library(GeneNet))
# clusterEvalQ(cl, library(magrittr))
# clusterEvalQ(cl, library(ppcor))
# 
# # generate results in parallel
# # for 50 bootstrapping -> time elapsed 72709.078 sec ~ 20h
# tic()
# ress <- parLapply(cl, nods, frun_and_gather_results_from_nod, nBoo = nboot, cut_vec = cut_vec)
# toc()
...
# remove results for pathways > 1000 genes
...
oo <- lapply(ress, function(x) x$res_opt$res_adjl$s$ntw_cut_offs_ps[-c(1:4),]) 
oo <- as.data.frame(do.call("rbind", oo[1:100,]))

data_plot <- as.data.frame(cbind(name=nn, cut_vec=cv, sign=is.sign, oo))
...
g_single <- lapply(names(ress)[pval.opt < log10(0.01/length(ress))], function(x) {
...
names(g_single) <- names(ress)[pval.opt < log10(0.01/length(ress))]
...
#save(data, net_data, file = "TranscriptomicsNetworkData.Rds")
